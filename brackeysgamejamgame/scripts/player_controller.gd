...
func _physics_process(delta: float) -> void:
	...
	# Step sounds
	if input.length() > 0 and not is_dashing and not step_sounds.playing and not bat:
		_play_step_sounds()
	elif (input.length() == 0 or is_dashing) and step_sounds.playing and not bat:
		_stop_step_sounds()
